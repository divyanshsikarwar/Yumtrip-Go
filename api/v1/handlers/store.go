...
func (s *Store) CreateStore(w http.ResponseWriter, r *http.Request) {
	...
	var storeAndCreds StoreAndCreds
	err := json.NewDecoder(r.Body).Decode(&storeAndCreds)
	if err != nil {
	...
	//Create the store
	err = storeAndCreds.Store.Create()
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	//Create the creds
	err = storeAndCreds.Creds.Create()
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
